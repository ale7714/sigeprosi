...
class BusquedaConCondicionOrdenada extends Busqueda{
		...
		Descripcion	: Constructor de la clase BusquedaConCondicionOrdenada.	*/
   		function __construct($nombre,$columnas,$parametros,$valores,$simbolo,$concat, $atribo,$orden) {
			...
			for ($i=0; $i<$N; $i++) {
				...
				if ($i != $N-1) {
					...
					$string= $string."$atributo"."$this->simbolo"."'$valor'";
                         ...
                         $string=$string." order by "."$this->atributorden"." "."$this->orden"." limit 1";
			return $string;
		}
